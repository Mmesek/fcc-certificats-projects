...
REGISTERED_PORTS = list(range(1, 1024))  # Task 5


def scan_port(port: int, target: IP) -> bool:  # Task 7.1
    ...
    if not response.haslayer(TCP):  # Task 11
        return False
    elif response.flags == 0x12:  # Task 12
        scapy.sr1(IP(dst=target) / TCP(sport=src_port, dport=port, flags="R"), timeout=2)  # Task 13
        return True


def is_available(target: IP) -> bool:  # Task 14
    try:  # Task 15
...
if __name__ == "__main__":
    ...
    for port in REGISTERED_PORTS:  # Task 21
        if scan_port(port, target):  # Task Sry, but variable here is useless, trust me 22
            open_ports.append(port)  # Task 23
            print(port)
    ...
    with open("PasswordList.txt", "r", newline="", encoding="utf-8") as file:  # Task 27
        passwords = file.readlines()  # Task 28
